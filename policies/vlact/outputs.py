"""VLAct episode files following the existing CogACT/X-VLA output format."""
...
def write_episode_artifacts(directory, record, task_config, block, timings, raw_chunks):
    ...
    write_json(path("_diagnostics.json"), {
        "format": "cogact_episode_v1", "policy": "vlact", "action_type": "qpos",
        "action_log_semantics": "Absolute Agilex joint targets: left 6 joints + gripper, right 6 joints + gripper. "
                                "Joint radians wrap to [-pi, pi); grippers clip to [-1, 1] as upstream.  EE fields are unavailable (null). "
                                "Remapped predictions, original model outputs and actually executed prefix are stored in _actions.npz.",

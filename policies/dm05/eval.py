#!/usr/bin/env python3
"""Exact-seed DM05 smoke runner: native task first, then one complete IF block."""
...
def _run_episode(env, config, client, args, seed, split, directory, block):
    ...
    import numpy as np
    from policies.dm05.client import CAMERAS, encode_proprio
...
def main():
    ...
    metadata = {"created_at": datetime.now(timezone.utc).isoformat(), "purpose": "integration_smoke",
                ...
                "source_sha256": {str(p.relative_to(REPO_ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                                  for p in [*Path(__file__).parent.glob("*.py"),
                                            ...
                                            REPO_ROOT / "policies/dm05/client.py"]},
    ...
    with (output / "console.log").open("w", buffering=1) as log, redirect_stdout(log), redirect_stderr(log):
        client = None
        try:
            from policies.dm05.client import DM05Client
            client = DM05Client(args.server_url, args.request_timeout)

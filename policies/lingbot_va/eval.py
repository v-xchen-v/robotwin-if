#!/usr/bin/env python3
"""Exact-seed LingBot-VA smoke runner: native task first, then one complete IF block."""
...
from policies.lingbot_va.outputs import write_episode_artifacts  # noqa: E402
...
def _run_episode(env, config, client, args, seed, split, directory, block):
    ...
    import numpy as np
    from policies.lingbot_va.client import CAMERAS, encode_proprio
    ...
    raw_chunks, skipped_frames, executed_actions, measured_poses, timings = [], [], [], [], []
    reset_started = False
    writer = None
    started = time.monotonic()
    stage = "oracle_setup"
    try:
        print(f"LingBot-VA task={args.task} seed={seed}", flush=True)
        random.seed(seed)
        ...
        while env.take_action_cnt < env.step_lim and not record["success"]:
            stage = "inference_or_cache_update"
            actions, prediction = client.predict(obs)
            raw_chunks.append(prediction["raw_actions"])
            skipped_frames.append(prediction["skipped_frames"])
            record["chunks"] += 1
        ...
        write_episode_artifacts(directory, record, args.task_config, block, timings, raw_chunks, skipped_frames)
...
def main():
    ...
    parser.add_argument("--server-url", default="ws://127.0.0.1:8011")
    ...
    with (output / "console.log").open("w", buffering=1) as log, redirect_stdout(log), redirect_stderr(log):
        client = None
        try:
            from policies.lingbot_va.client import LingBotClient
            client = LingBotClient(args.server_url, args.request_timeout)

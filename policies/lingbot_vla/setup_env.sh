...
LINGBOT_ENV_NAME="robotwin-if-lingbot-vla"
LINGBOT_SOURCE_DIR="$REPO_ROOT/third_party/lingbot-vla"
LINGBOT_REVISION="4eb34b7693a0565c67433f8fac9c59a2e67eb60b"

usage() {
    cat <<'EOF'
Usage: bash policies/lingbot_vla/setup_env.sh [--env-name NAME] [--source-dir PATH]

Create/reuse a dedicated Python 3.12 environment, fetch pinned official source,
enable its Torch attention backend without requiring FlashAttention, install
CUDA 12.8 inference dependencies and check imports. No weights are downloaded.
...
case "${LINGBOT_ENV_NAME,,}" in
    base|root|robotwin|xvla|robotwin-if-xvla|robotwin-if-lingbot-va) fail "Use a dedicated LingBot-VLA environment" ;;
esac
...
if [[ ! -e "$LINGBOT_SOURCE_DIR" ]]; then
    ...
    git -C "$LINGBOT_SOURCE_DIR" remote add origin https://github.com/Robbyant/lingbot-vla.git
...
if [[ -n "$env_prefix" ]]; then
    conda run --no-capture-output -n "$LINGBOT_ENV_NAME" python -c 'import sys; assert sys.version_info[:2] == (3, 12), "Python 3.12 required"'
else
    conda create --yes --name "$LINGBOT_ENV_NAME" python=3.12 pip
fi

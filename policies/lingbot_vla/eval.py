#!/usr/bin/env python3
"""Exact-seed LingBot-VLA smoke runner: native task first, then one complete IF block."""
...
from policies.lingbot_vla.outputs import write_episode_artifacts  # noqa: E402
...
def _run_episode(env, config, client, args, seed, split, directory, block):
    ...
    import numpy as np
    from policies.lingbot_vla.client import CAMERAS, encode_proprio
    ...
    raw_chunks, executed_actions, measured_poses, measured_joints, timings = [], [], [], [], []
    reset_started = False
    writer = None
    started = time.monotonic()
    stage = "episode_setup"
    try:
        print(f"LingBot-VLA task={args.task} seed={seed}", flush=True)
...
def main():
    ...
    parser.add_argument("--server-url", default="ws://127.0.0.1:8012")
    ...
    with (output / "console.log").open("w", buffering=1) as log, redirect_stdout(log), redirect_stderr(log):
        client = None
        try:
            from policies.lingbot_vla.client import LingBotVLAClient
            client = LingBotVLAClient(args.server_url, args.request_timeout)
            metadata["server_metadata"] = client.server_metadata
            write_json(output / "run.json", metadata)
            env, config = load_task(target, args.task, args.task_config)
            config["policy_name"] = "lingbot_vla"

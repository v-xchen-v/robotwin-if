...
PANELS = [('Xa — Selection / Grounding', list(TASKS)[:4]),
          ('Xb — Structured Execution', list(TASKS)[4:])]
...
def balanced_row(spec, source, records):
    ...
    task = spec['task']
    contract = IF_SEED_CONTRACTS[task]
    ...
    columns = []
    for label, keys in TASKS[task][1]:
...
def snapshot(base):
    ...
    assert {r['task'] for r in plan['tasks']} == set(TASKS)
    ...
    for row in source['tasks']:
        ...
        for seed in spec['seeds']:
            ...
            assert marker['formal_block'] == spec['seeds'].index(seed)//IF_SEED_CONTRACTS[row['task']].block_size
            records[seed] = r
...
def generate(data, output):
    ...
    overview = (f"快照：{data['updated_at']}；完成 {source['completed_episodes']}/{source['expected_episodes']} 回合，"
                ...
                f"跑齐任务组合 {source['complete_task_policy_runs']}/42。")
    legend = [
        '沿用 results-table-draft.md 的 policy × task/mode 两 panel 结构；任务版本为 arm select v2、grasp approach v2 wide-r1。',
        '模式单元格为 SR (%)（成功数/纳入统计回合数）；0.0 表示已评测但没有成功，— 表示尚无完整 block 可统计。',
        '每项进度为完整 blocks/12 B、全部有效已完成回合/计划 ep。成功与 policy failure 均计入已完成；缺失/运行错误不计入。',
        ...
        '† 表示该任务未跑齐 12 blocks，当前 Avg. 为暂定；各 policy 暂定分数可能基于不同 seed 子集，不作最终排名。',
        'Overall 仅在该 policy 的七项任务全部跑齐后填写，按七个 Task Avg. 等权平均，绝不把所有回合混算。',
        'Sequence 缩写为从下往上的颜色顺序；Spatial Top 指 on_top，Grasp Top 指顶部抓取。',
    ]
    ...
    for policy, label in POLICIES.items():
        ...
        md.append(f"| {label} | {p['recorded']}/{p['expected']} | {p['complete_tasks']}/7 | "
                  f"{p['complete_blocks']}/{p['expected_blocks']} | {overall} |")
    panels = []
    for title, tasks in PANELS:
        md += ['', '## '+title]
        first = '<tr><th rowspan="2">Policy</th>'
        second = '<tr>'
        for task in tasks:
            label, cols = TASKS[task]
...
def main():
    ...
    data = snapshot(args.run_dir.resolve())

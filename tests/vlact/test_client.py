...
from policies.vlact.client import (  # noqa: E402
    CAMERAS, PROTOCOL, VLActClient, decode_actions, packb, unpackb,
)
from policies.vlact.eval import run_episode  # noqa: E402
...
def chunk():
    return np.broadcast_to(np.arange(14, dtype=np.float32) / 20, (32, 14)).copy()


def client_with_responses(*responses, metadata=None):
    conn = Mock()
    metadata = metadata if metadata is not None else {"ok": True, "protocol": PROTOCOL, "use_length": 32,
                                                                "action_type": "qpos", "joint_action_mode": "wrap"}
    def response(r):
        if isinstance(r, str):
            return r
        if "action" in r:
            return packb({"ok": True, "data": {"action": r["action"], "model_actions": r["action"]}})
        return packb({"ok": True, **r})
    conn.recv.side_effect = [packb(metadata)] + [response(r) for r in responses]
    with patch("policies.vlact.client.connect", return_value=conn):
        client = VLActClient()
    return client, conn
...
class ClientTests(unittest.TestCase):
    def test_absolute_joint_order_and_continuous_grippers(self):
        ...
        for bad in (np.zeros((32, 16)), np.zeros((0, 14)), np.zeros((33, 14)), np.full((32, 14), np.nan)):
    ...
    def test_reset_and_fresh_observation_each_chunk(self):
        ...
        client.predict(observation(32))
        sent = [unpackb(call.args[0]) for call in conn.send.call_args_list]
        self.assertEqual(sent[0], {"type": "reset", "seed": 100001, "instruction": "Use the right arm"})
        self.assertEqual(sent[1]["examples"][0]["lang"], "Use the right arm")
        self.assertNotIn("state", sent[2]["examples"][0])
        for i, rgb in enumerate(sent[2]["examples"][0]["image"]):
            self.assertEqual(rgb[0, 0, 0], 32 + i)
    ...
    def test_wrong_protocol_and_invalid_requests_fail_closed(self):
        for metadata in ({}, {"protocol": "lingbot_va_robotwin_v1"},
                         ...
                         {"protocol": PROTOCOL, "use_length": 32, "action_type": "ee"}):
            with self.assertRaises((ValueError, RuntimeError)):
                client_with_responses(metadata={"ok": True, **metadata})
    ...
    def test_wire_format_matches_official_numpy_codec(self):
        source = ROOT / "third_party/vlact/deployment/model_server/tools/msgpack_numpy.py"
        ...
        for success_at, steps in ((5, 5), (100, 53)):
            with self.subTest(steps=steps), tempfile.TemporaryDirectory() as tmp:
                ...
                with np.load(directory / (prefix + "_actions.npz")) as trace:
                    ...
                    self.assertEqual(trace["raw_actions"].shape, (1 if steps == 5 else 2, 32, 14))
                    np.testing.assert_array_equal(trace["executed_actions"], np.tile(chunk()[0], (steps, 1)))
                    np.testing.assert_array_equal(trace["model_actions"], trace["raw_actions"])
                ...
                self.assertEqual(len(logs[0]["ROBOT_LEFT_JOINT_POS"]), 32)

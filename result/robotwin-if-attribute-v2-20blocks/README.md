# RoboTwin-IF Attribute-v2：六任务、六 policies、20 blocks

已完成并校验 **2760/2760 回合、720/720 blocks**。共 1103 次成功、1657 次 policy failure。
...
共有 1 次推理前 oracle 初始化失败，均在新 simulator 中用原 seed 恢复；
...
- [新 taskset](../../seed-manifests/robotwin-if-attribute-v2-20-per-mode/README.md) · [新判据与回放](../../docs/attribute-select-target-only.md)
...
原始视频与动作：`/Data/robotwin-if/evaluations/robotwin-if-attribute-v2-20blocks-001/<policy>/<task>/`。
视频复核：`python tools/policy-video-review/server.py --run-dir /Data/robotwin-if/evaluations/robotwin-if-attribute-v2-20blocks-001`。

...
- **指令示例**："把它合上" / "把它打开"。**已实现为两个并列 task**（RoboTwin 指令路由靠占位符集合精确匹配，开/关都只有 {A}/{a}、无天然区分占位符，故不合成单 env、而是同场景两 task_name）：`tasks/task_instruction/close_laptop.json`（关向措辞新写，句式逐条平行于 native open、仅反动词，遵循 [[reuse-native-instruction-pools]]）与 `open_laptop_mid.json`（开向借 native `open_laptop.json`）。两池渲染正常、只有动词不同。
- **成功判定**：初始态为**开、关共用的中间开度（~50%）**——两个变体初始画面完全相同，只有指令动词不同，逼 policy 读动词而非靠初始几何猜动作（呼应"动词-物体解耦"的测试目标，也让 50% 半开这个歧义最大的状态把动词先验暴露得最彻底）。铰链关节角到达指令方向对应端——开 → qpos 过高阈值（**~70%**，见下方 spike：开向可靠上限 ~78%，够不到 ~90%）；关 → qpos 回到低阈值区间（~10–15%）；**不是**"关节角发生了变化"。50% 起点 → 开 ~78% / 关 ~5% 三档在画面上清楚可分。
- **反例设计（Layer B，已过）**：`tests/close_laptop/test_check_success.py` 直接把铰链 `set_qpos` 到各末态断言 check_success——关键反例"应关却开(~78%)→ close 判 False""应开却关(~5%)→ open 判 False"均过，另含正例/未动(50%)/阈值两侧,共 10/10。
